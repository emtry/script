...
// 判断是否登录 
axios_V2EX.get('/', {}).then(function(response) {
    if (response.status == 200) {
        let regex = />登录</;
        let m = regex.exec(response.data);
        if (m) {
            console.log('未登录\n');
            notify.sendNotify('V2EX_daily', '未登录');
        } else {
            // 判断是否已签到
            axios_V2EX.get('/mission/daily', {}).then(function(response) {
                if (response.status == 200) {
                    let regex = /每日登录奖励已领取/;
                    let m = regex.exec(response.data);
                    if (m) {
                        ...
                        if (response.status == 200) {
                            ...
                            if (m) {
                                console.log(m[0] + '\n');
                                notify.sendNotify('V2EX_daily', '今日已签到\n' + m[0]);
                            }
                        } else {
                            console.log('未知错误');
                        ...
                        // 签到
                        axios_V2EX.get(m[1], {}).then(function(response) {
                            if (response.status == 200) {
                                let regex = /已连续登录 [1-9]* 天/;
                                let m = regex.exec(response.data);
                                if (m) {
                                    console.log('签到成功\n' + m[0] + '\n');
                                    notify.sendNotify('V2EX_daily', '签到成功\n' + m[0]);
                                }
                            } else {
                                console.log('未知错误');
                                console.log(response.data);
